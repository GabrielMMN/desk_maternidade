...
class SimulationReporter:
    ...
    def _print_entity_counts(self):
        """Print entity creation and disposal counts."""
        total_created = sum(getattr(block, 'entities_created', 0)     # MODIFICADO: Agora busca por criações
                            for block in self.model.blocks.values())  # em qualquer bloco com contador self.entities_created
        # total_disposed = sum(
        #     len([e for e in block.disposed_entities 
        #          if e.get_attribute('disposal_time', 0) >=  # ATENÇÃO: só computa a saida se for depois do warm-up
        #          self.model.warm_up_period])
        #     for block in self.model.dispose_blocks
        # )
        total_disposed = sum(len(block.disposed_entities)   # Agora computa TODAS as saídas (até no warm-up)
                             for block in self.model.dispose_blocks)
