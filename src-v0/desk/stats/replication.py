...
class ReplicationFramework:
    ...
    def run_replications(self, base_seed: int = 12345, **simulation_kwargs):
        ...
        # TESTE GRÁFICOS ADICIONAIS
        # self.plot_kpi_confidence_intervals()
    ...
    # ADICIONADO
    # def plot_kpi_confidence_intervals(self, metrics_to_plot=None):
    #     if not self.summary_statistics:
    #         print("Statistics not computed.")
    #         return

    #     if metrics_to_plot is None:
    #         metrics_to_plot = [k for k in self.summary_statistics if not k.endswith('_utilization')]

    #     # Filtrar métricas que existem e que fazem sentido para plotar
    #     available = [m for m in metrics_to_plot if m in self.summary_statistics]
    #     values = [self.summary_statistics[m]['mean'] for m in available]
    #     errors = [self.summary_statistics[m]['half_width'] for m in available]

    #     fig, ax = plt.subplots(figsize=(12, 6))
    #     ax.barh(available, values, xerr=errors, capsize=5)
    #     ax.set_title('Intervalos de confiança para métricas de réplica')
    #     plt.tight_layout()
    #     plt.show()
