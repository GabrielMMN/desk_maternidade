...
class DecideBlock(BaseBlock):
    ...
    def process_entity(self, entity: Entity):
        ...
        if chosen_route and chosen_route in self.routes:
            ...
            # --- INÍCIO DA MODIFICAÇÃO: Filtro de Warm-up ---
            # Busca o tempo de aquecimento (se o modelo não tiver, assume 0)
            warm_up = getattr(self.env.model, 'warm_up_period', 0) if hasattr(self.env, 'model') else 0
            
            # Só incrementa o contador se o relógio já passou do aquecimento
